...
class Omeka_DbTest extends PHPUnit_Framework_TestCase
{
	...
	public function testDbCanRetrieveTableNameWithPrefix()
	{
	    ...
	    $this->_adapter = $this->getMock('Zend_Db_Adapter_Mysqli', array(), array(), '', false);

		$db_with_prefix = new Omeka_Db($this->_adapter, 'foobar_');
		
		$this->assertEquals($db_with_prefix->Item, 'foobar_items');
		
		$db_without_prefix = new Omeka_Db($this->_adapter);
